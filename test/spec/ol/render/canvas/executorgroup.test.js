...
describe('ol.render.canvas.ExecutorGroup', function () {
  describe('#getPixelIndexArray', function () {
    ...
    it('orders the indexes correctly from closest to farthest away', function () {
      ...
      const distOne = [
        (radius * size + radius + 1) * 4 + 3,
        ((radius + 1) * size + radius) * 4 + 3,
        ((radius - 1) * size + radius) * 4 + 3,
        (radius * size + radius - 1) * 4 + 3,
      ];
      expect(distOne).to.contain(hitIndexes[1]);
      expect(distOne).to.contain(hitIndexes[2]);
      expect(distOne).to.contain(hitIndexes[3]);
      expect(distOne).to.contain(hitIndexes[4]);

      // 4 Pixels in the middle of each side last
      const distTen = [
        (0 * size + radius) * 4 + 3,
        (radius * size + 0) * 4 + 3,
        (radius * size + size - 1) * 4 + 3,
        ((size - 1) * size + radius) * 4 + 3,
      ];
      expect(distTen).to.contain(hitIndexes[hitIndexes.length - 1]);
      expect(distTen).to.contain(hitIndexes[hitIndexes.length - 2]);
      expect(distTen).to.contain(hitIndexes[hitIndexes.length - 3]);
      expect(distTen).to.contain(hitIndexes[hitIndexes.length - 4]);
    });
