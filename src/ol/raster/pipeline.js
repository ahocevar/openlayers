 ...
 */
export function compileStyle(style, bandCount, nodataBandIndex) {
  /**
  ...
  function number(encoded) {
    ...
    return /** @type {import('../expr/cpu.js').NumberEvaluator} */ (
      buildExpression(encoded, NumberType, newParsingContext())
    );
  }
  ...
  return {
    color:
      style.color === undefined
        ? null
        : buildExpression(style.color, ColorType, newParsingContext()),
...
/**
 * One tile's worth of work, as it crosses the wire.  The style travels with every job, so
 * any worker in the pool can serve any tile without configure and render messages having
...
/**
 * Make a function that turns a job into a reply.  The worker and the main thread run the
 * same one, so the message contract has a single implementation.
 *
...
export function createJobHandler() {
  ...
  return async function (job) {
    try {
      ...
      // An ImageBitmap draws without a canvas of its own, which keeps the styled tiles out
      // of the 2d canvas memory browsers cap.
      ...
      // Compiling can fail on a malformed style, and evaluating can fail on a band index
      // the data does not have.  Report it rather than leaving the tile pending forever.
      pipeline = null;
      pipelineId = null;
