...
/**
 * A worker, or something that behaves like one.  The faux worker used when
 * `threads` is 0 implements the same three members, so the pool does not need
...
/**
 * A unit of work.  A job holds one message per worker it needs, and is only
 * dispatched once that many workers are free.  A job with as many messages as
 * there are workers therefore runs alone, while single-message jobs run
 * concurrently, up to one per worker.
 *
 * @typedef {Object} Job
...
/**
 * @classdesc
 * A pool of workers.  Used by {@link module:ol/source/Raster~RasterSource} to run pixel and
 * image operations, and by the canvas tile renderer to apply raster styles to data tiles.
 */
class Processor extends Disposable {
  ...
  constructor(config) {
    ...
    // `threads: 0` still needs somewhere to run the work, so the pool always
    // holds at least one (possibly faux) worker.
   ...
   * pool.  Because such a job occupies all workers, only one runs at a time.
   *
  ...
  /**
   * Queue a job.  A job with a single message runs on the next free worker, so up to
   * one job per worker runs concurrently.  A job with as many messages as there are
   * workers occupies the whole pool and runs on its own.
   *
  ...
  /**
   * Give up the pool, telling everything still queued or running that it will get no reply.
   * Unlike {@link Processor#dispose}, which stops callbacks without a word, this lets a caller
   * that still wants its result ask for it somewhere else — which is what replacing a pool
   * needs, since work in flight has done nothing wrong.
   */
  abandon() {
