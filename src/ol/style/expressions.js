 ...
 *
 * * Reading operators:
 *   * `['band', bandIndex, offsetX, offsetY]` fetches pixel values from band `bandIndex` of the source's
 *     data. The first `bandIndex` of the source data is `1`. Fetched values are in the 0..1 range.
 ...
 *     {@link import("../source/GeoTIFF.js").Options configuration}. `offsetX` and `offsetY` are optional
 *     and allow specifying pixel offsets for x and y. This is used for sampling neighboring pixels.
  ...
  toGlsl: function (context, args) {
    ...
    if (args.length === 1) {
      ...
      return `texture2D(${uniformName}, v_textureCoord + vec2(${expressionToGlsl(
        context,
        xOffset
      )} / ${Uniforms.PIXEL_SIZE_X}, ${expressionToGlsl(context, yOffset)} / ${
        Uniforms.PIXEL_SIZE_Y
      }))[${bandIndex}]`;
    }
  },
};
