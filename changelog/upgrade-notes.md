...
#### Interpolated color channels are no longer rounded to integers

Channels keep their fractional part, so operations applied on top of an interpolated color
are more accurate. Rendering is unaffected, but code reading a color out of an expression may
now see a fractional channel.
