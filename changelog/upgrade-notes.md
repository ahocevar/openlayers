...
Interpolated color channels are no longer rounded to integers, so that a raster pipeline
applying gamma or contrast after a color ramp does not compound a rounding error at every
stop. Colors are quantized where they are written, as `mix()` does in a shader. This is
invisible to rendering, but code reading a color out of an expression may now see a
fractional channel.
